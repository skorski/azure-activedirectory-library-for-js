...
var AuthenticationContext = (function () {
     ...
     *  @property {string} tokenProxy - The proxy server that will add the clientSecret to the post requets
     ...
     *  @property {string} resource - The Azure resource where you are requesting access.  example: 'https://management.core.windows.net/'
     *  @property {string} clientSecret - A secret for the client.
    ...
    AuthenticationContext.prototype.login = function () {
        ...
        if (this.config.resource) {
            urlNavigate += '&resource=' + encodeURIComponent(this.config.resource);
        }
    ...
    AuthenticationContext.prototype._createUser = function (idToken) {
        ...
        if (idToken){
            console.log('There is a token that can be parsed');
        }
        if (parsedJson && parsedJson.hasOwnProperty('aud')) {
            if (parsedJson.aud.toLowerCase() === this.config.clientId.toLowerCase()) {
                ...
                this.warn('different audience... this may be desired');
                var userName = '';
                if(parsedJson.hasOwnProperty('given_name')){
                    userName += parsedJson.given_name;
                }
                if(parsedJson.hasOwnProperty('family_name')){
                    userName += ' ' + parsedJson.family_name;
                }
                var groups = [];
                if(parsedJson.hasOwnProperty('groups')){
                ...
                user = {
                    userName: userName,
                    groups: groups,
                    audience: parsedJson.aud,
                    nonce: this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN),
                    profile: parsedJson
                };

                user.profile.nonce = this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN);

                if (parsedJson.hasOwnProperty('upn')) {
                    user.userName = parsedJson.upn;
                } else if (parsedJson.hasOwnProperty('unique_name')) {
                    user.userName = parsedJson.unique_name;
                }
    ...
    AuthenticationContext.prototype.getRequestInfo = function (hash) {
        ...
        if (parameters) {
            ...
            if (parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
                ...
                if (parameters.hasOwnProperty('requestType')){
                    // this is a token captured from a code
                    requestInfo.requestType = parameters.requestType;
                }
        ...
        if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
            ...
            if (requestInfo.stateMatch) {
                ...
                if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                    ...
                    var accessToken = this.getTokenFromAccessCode(requestInfo.parameters);
                ...
                if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                    ...
                    if (requestInfo.parameters.nonce ){
                        // the nonce was manually passed by the redirect
                        this._user.nonce = requestInfo.parameters.nonce;
                    }
                    if (this._user && this._user.profile) {
    ...
    AuthenticationContext.prototype.getTokenFromAccessCode = function (parameters){
        var data = {
        ...
        const nonce = this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN);
        //TODO: Replace this call with a proxy so the client secret can be added without the developer (or client) seeing it.
        console.log(data);
        jQuery.ajax({
            url: this.config.tokenProxy + this.config.tenant + '/oauth2/token',
            type: 'post',
            data: data,
            headers: {
            ...
            success: function(res){
                console.log(res);
                console.log(window.location.href.split('?')[0].split('#')[0] + '/#id_token=' + res.access_token);
